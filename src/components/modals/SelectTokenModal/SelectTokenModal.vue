...
const tokens = computed(() => {
  let tokensWithValues : List[] = [];
  Object.values(state.results).map(token => {
    if(XTokenLists.indexOf(token.address) > -1) {
      const balance = balanceFor(token.address);
      const price = priceFor(token.address);
      const value = Number(balance) * price;

      let data = {...token, price, balance, value};
      let _data = {
